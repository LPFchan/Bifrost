...
## 3. Realtime sync with GitHub Actions

To keep the Vertex AI Search data store in sync with the repo on every push, use the included GitHub Actions workflow.

### One-time setup

1. **Create GitHub secrets** (repo → Settings → Secrets and variables → Actions):
   - **`GCP_SA_KEY`** – Full contents of your service account JSON key file (e.g. the key for `cline-vertex@...`). Paste the entire JSON.
   - **`GCP_PROJECT_ID`** – Your Google Cloud project ID (e.g. `lostplusfound`).
   - **`BIFROST_GCS_BUCKET`** (optional) – GCS bucket name; defaults to `bifrost-bucket` if not set.

2. **Ensure the service account** has:
   - `roles/discoveryengine.admin`
   - `roles/storage.objectAdmin` (on the bucket or project)
   The data store and bucket must already exist (create them once locally or in the Console).

3. **Workflow file** – `.github/workflows/bifrost-sync.yml` is already in the repo. It runs on every push to `main` and on manual trigger (Actions → Bifrost sync to Vertex AI Search → Run workflow).

### What it does

On each push to `main`, the workflow checks out the repo, installs Bifrost dependencies, authenticates with `GCP_SA_KEY`, and runs Bifrost with `--skip-create-datastore` on the current repository (`.`). The Vertex AI Search index is updated incrementally with the latest commit.
